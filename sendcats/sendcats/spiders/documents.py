...
class CoursesSpider(scrapy.Spider):
    ...
    def parse_course(self, response):
        ...
        for course in course_list:
            url = u'http://www.sendcats.com/resource/course?course_id={}&page=1'
            url = url.format(course['course_id'])
    ...
    def parse_document(self, response):
        ...
        if data['err'] != 0:
            logging.warning('failed request: {}'.format(response.url))
            return
        doc_list = data['resource_list']
        if not doc_list:
            return
        yield scrapy.Request(self.get_next_url(response.url), self.parse_document)
        for doc in doc_list:
            yield doc
