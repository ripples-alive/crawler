...
def single_download(stop_id, last_id=None):
    ...
    sql = 'SELECT * FROM diyifanwen_document WHERE id > %s AND id <= %s ORDER BY id LIMIT 1'
    document = db.get(sql, last_id, stop_id)
    if document is None:
        return stop_id

    try:
        log.info('downloading: %d %s', document['id'], document['title'])
    except Exception as e:
    ...
    try:
        if document['download_url']:
            ...
            switch_proxy()
            headers = {
                'Referer': 'http://www.diyifanwen.com',
            }
            url = 'http://s.diyifanwen.com/down/?action=down&id={}'.format(document['post_url'])
            get_response(url, headers=headers)
            url = 'http://s.diyifanwen.com/down/doc.asp'
            res = get_response(url, headers=headers)

        filename = re.search('filename=(.*)', res.headers['Content-Disposition']).group(1)
        try:
            filename = filename.decode('gbk')
        except UnicodeDecodeError as err:
            filename = '.' + filename.split('.')[-1]
        filename = u'{0[id]:d}-{0[title]}-{1}'.format(document, filename)
    ...
    sql = 'UPDATE diyifanwen_document SET state = %s WHERE id = %s'
...
def daemon_download(daemon_id, interval=60):
    db.update('UPDATE diyifanwen_document SET state = 0 WHERE state = %s', -daemon_id)
    while True:
        affected_rows = db.update(
            'UPDATE diyifanwen_document SET state = %s WHERE state = 0 LIMIT 1',
            -daemon_id)
        if affected_rows == 0:
        ...
        stdoc = db.get(
            'SELECT id FROM diyifanwen_document WHERE state = %s LIMIT 1', -daemon_id)
